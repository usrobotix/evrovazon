...
(function(window, document){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    if(btn && ov){
      ...
      /* Переключение панелей каталога по ховеру/клику на категорию */
      ov.querySelectorAll('.ev-catlist a').forEach(function(a){
        var activate = function(){
          var id = a.getAttribute('data-cat-id');
          if(!id) return;
          ov.querySelectorAll('.ev-catlist a').forEach(function(x){ x.classList.remove('is-active'); });
          ov.querySelectorAll('.ev-catpane').forEach(function(p){ p.classList.remove('is-active'); });
          a.classList.add('is-active');
          var pane = ov.querySelector('.ev-catpane[data-cat-pane="' + id + '"]');
